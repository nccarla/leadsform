...
app.post('/api/audit', async (req, res) => {
    const { client_id, nombre, correo, telefono, asunto, ubicacion, fecha_inicio, fecha_fin, descripcion, validador, pais, dia_reunion, asesor, } = (req.body ?? {});
    try {
        const query = `
      INSERT INTO audits (
...
app.patch('/api/audit/:id/advisor-name', async (req, res) => {
    const { id } = req.params;
    const body = (req.body ?? {});
    const rawName = body.advisor_name ?? body.asesor ?? body.advisorName ?? body.name ?? null;
...
app.get('/api/state', async (_req, res) => {
    try {
        const { rows } = await pool.query('SELECT payload FROM lead_app_state WHERE id = 1');
        const row = rows[0];
        if (!row) {
...
app.put('/api/state', async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE lead_app_state SET payload = $1::jsonb, updated_at = now() WHERE id = 1`, [JSON.stringify(body)]);
        res.json({ ok: true });
    }
    catch (e) {
        console.error(e);
...
app.get('/api/opportunity', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT opportunity_number, client_name, client_email, client_phone, seller_name, updated_at
       FROM opportunity_directory
       WHERE opportunity_number = $1`, [num]);
        const row = rows[0];
        if (!row) {
...
app.put('/api/opportunity', async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO opportunity_directory (
       ...
       ON CONFLICT (opportunity_number) DO UPDATE SET
         ...
         updated_at = now()`, [opportunityNumber, clientName, clientEmail, clientPhone, sellerName]);
        res.json({ ok: true });
    }
    catch (e) {
        console.error(e);
...
app.get('/api/history', async (req, res) => {
    try {
        ...
        const entries = rawHistory.filter((item) => {
...
async function main() {
    ...
    startMeetingAuditor();
    app.listen(PORT, () => {
        console.log(`API PostgreSQL http://localhost:${PORT} (GET/PUT /api/state, GET /api/history, GET/PUT /api/opportunity, POST /api/audit, PATCH /api/audit/:id/status)`);
    });
}
main().catch((e) => {
    console.error(e);
